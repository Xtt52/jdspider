...
class JdspiderSpider(scrapy.Spider):
    ...
    def parse(self, response):
        self.brands_url_list = response.css('ul#brandsArea li a::attr(href)').extract()
        self.brands_url = self.brands_url_list[0:8]
        for self.brand_url in self.brands_url:
            yield Request(url=parse.urljoin(response.url, self.brand_url), callback=self.parse_brand)
        # yield Request(url=parse.urljoin(response.url, self.brands_url.pop()), callback=self.parse_brand)

    def parse_brand(self, response):
        next_urls = response.css('a.pn-next::attr(href)').extract()[0]
        if next_urls:
            # self.page_count = self.page_count-1
            yield Request(url=parse.urljoin(response.url, next_urls), callback=self.parse_brand)
